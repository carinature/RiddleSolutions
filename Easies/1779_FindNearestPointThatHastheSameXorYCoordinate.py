...
def nearestValidPoint(x: int, y: int, points: List[List[int]]) -> int:
    ...
    for i, (xx, yy) in enumerate(points):
        xr, yr = x - xx, y - yy
        d = abs(xr) + abs(yr)
        if not xr * yr and dmin > d:
            imin, dmin = i, d
    return imin  # this also covers cases of


def testing():
    result = nearestValidPoint(x=3, y=4,
                               points=[[1, 2], [3, 1], [2, 4], [2, 3], [4, 4]])
    # print(f"result: {result}")
    assert (2 == result)

    result = nearestValidPoint(x=3, y=4, points=[[3, 4]])
    # print(f"result: {result}")
    assert (0 == result)

    result = nearestValidPoint(x=3, y=4, points=[[2, 3]])
    # print(f"result: {result}")
    assert (-1 == result)
